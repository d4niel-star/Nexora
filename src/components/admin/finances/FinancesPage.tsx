  ...
  return (
    <div className="animate-in fade-in space-y-8 pb-32 duration-700">
      ...
      <div className="relative overflow-hidden rounded-[var(--r-md)] border border-[color:var(--hairline)] bg-[var(--surface-0)] ">
        <div aria-label="Secciones de finanzas" className="flex items-center gap-8 overflow-x-auto border-b border-[color:var(--hairline)] bg-[var(--surface-1)] px-6 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden" role="tablist">
          {tabs.map((tab) => (
            <button key={tab.value} aria-selected={activeTab === tab.value} className={cn("group relative whitespace-nowrap py-4 text-[13px] font-semibold transition-colors focus-visible:outline-none focus-visible:shadow-[var(--shadow-focus)]", activeTab === tab.value ? "text-ink-0" : "text-ink-5 hover:text-ink-0")} onClick={() => handleTabChange(tab.value)} role="tab" type="button">
          ...
          <div className="flex flex-col gap-4 border-b border-[color:var(--hairline)] bg-[var(--surface-0)] p-4 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex w-full flex-col gap-3 lg:flex-row lg:items-center">
              <div className="group relative w-full lg:max-w-sm">
                <Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-6 transition-colors group-focus-within:text-ink-0" />
                <input className="w-full rounded-[var(--r-sm)] border border-transparent bg-[var(--surface-1)] py-2.5 pl-10 pr-4 text-[13px] font-medium text-ink-0 transition-all placeholder:text-ink-6 focus:border-[var(--accent-500)] focus:bg-[var(--surface-0)] focus:outline-none focus-visible:shadow-[var(--shadow-focus)]" onChange={(e) => setSearchQuery(e.target.value)} placeholder="Buscar referencia, cliente..." type="text" value={searchQuery} />
              </div>
              {showStatusFilter ? (
        ...
        <div className="min-h-[420px] bg-[var(--surface-0)]">
          {isLoading ? (
            <TableSkeleton />
          ) : activeTab === "resumen" ? (
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <NavCard icon={<Banknote className="h-4 w-4" strokeWidth={1.75} />} title="Cobrado" description={`${data.movements.length} movimientos`} onClick={() => onNavigate("cobrado")} />
        <NavCard icon={<Clock className="h-4 w-4" strokeWidth={1.75} />} title="Pendiente" description={`${data.pending.length} por cobrar`} onClick={() => onNavigate("pendiente")} />
      ...
      {data.pending.filter(p => p.status === "critical").length > 0 || data.margins.filter(m => m.health === "critical").length > 0 ? (
        <div className="space-y-3">
          <h3 className="text-[11px] font-semibold uppercase tracking-[0.18em] text-ink-5">Alertas financieras</h3>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {data.pending.filter(p => p.status === "critical").slice(0, 2).map((p) => (
              <button key={p.id} className="flex items-start gap-3 rounded-[var(--r-md)] border border-[color:var(--hairline)] bg-[var(--surface-0)] p-4 text-left  transition-all focus-visible:outline-none focus-visible:shadow-[var(--shadow-focus)]" onClick={() => openDrawer({ kind: "pending", data: p })} type="button">
                <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--r-sm)] border border-[color:var(--hairline)] bg-[var(--surface-1)]"><AlertTriangle className="h-3.5 w-3.5 text-[color:var(--signal-danger)]" strokeWidth={1.75} /></div>
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-ink-0">{p.reference}</p>
                  <p className="mt-1 line-clamp-1 text-xs font-medium text-ink-5">{p.cause} — {formatCurrency(p.amount)}</p>
                </div>
              </button>
            ))}
            {data.margins.filter(m => m.health === "critical").slice(0, 2).map((m) => (
              <button key={m.id} className="flex items-start gap-3 rounded-[var(--r-md)] border border-[color:var(--hairline)] bg-[var(--surface-0)] p-4 text-left  transition-all focus-visible:outline-none focus-visible:shadow-[var(--shadow-focus)]" onClick={() => openDrawer({ kind: "margin", data: m })} type="button">
                <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--r-sm)] border border-[color:var(--hairline)] bg-[var(--surface-1)]"><TrendingUp className="h-3.5 w-3.5 text-[color:var(--signal-warning)]" strokeWidth={1.75} /></div>
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-ink-0">{m.name}</p>
                  <p className="mt-1 line-clamp-1 text-xs font-medium text-ink-5">Margen critico: {m.marginPercent}%</p>
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1000px] text-left">
          ...
          <tbody className="divide-y divide-[color:var(--hairline)]">
            {filtered.map((m) => (
              <tr key={m.id} className="group cursor-pointer bg-[var(--surface-0)] transition-colors hover:bg-[var(--surface-1)]/60 focus-within:bg-[var(--surface-1)]/80" onClick={() => openDrawer({ kind: "movement", data: m })} tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openDrawer({ kind: "movement", data: m }); } }}>
                <td className="px-6 py-4 text-sm font-semibold text-ink-0">{m.reference}</td>
                <td className="px-6 py-4 text-sm font-medium text-ink-0">{m.customer}</td>
                <td className="px-6 py-4"><ChannelBadge channel={m.channel} /></td>
                <td className="px-6 py-4 text-xs font-semibold tabular-nums text-ink-5">{timeFormatter.format(new Date(m.date))}</td>
                <td className="px-6 py-4 text-sm font-semibold tabular-nums text-ink-0">{formatCurrency(m.gross)}</td>
                <td className="px-6 py-4 text-sm font-medium tabular-nums text-ink-5">{formatCurrency(m.commission)}</td>
                <td className="px-6 py-4 text-sm font-medium tabular-nums text-ink-5">{formatCurrency(m.shipping)}</td>
                <td className="px-6 py-4 text-sm font-semibold tabular-nums text-[color:var(--signal-success)]">{formatCurrency(m.net)}</td>
    ...
    <div className="overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[900px] text-left">
          ...
          <tbody className="divide-y divide-[color:var(--hairline)]">
            {filtered.map((p) => (
              <tr key={p.id} className="group cursor-pointer bg-[var(--surface-0)] transition-colors hover:bg-[var(--surface-1)]/60 focus-within:bg-[var(--surface-1)]/80" onClick={() => openDrawer({ kind: "pending", data: p })} tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openDrawer({ kind: "pending", data: p }); } }}>
                <td className="px-6 py-4 text-sm font-semibold text-ink-0">{p.reference}</td>
                <td className="px-6 py-4 text-sm font-medium text-ink-0">{p.customer}</td>
                <td className="px-6 py-4 text-xs font-semibold tabular-nums text-ink-5">{timeFormatter.format(new Date(p.date))}</td>
                <td className="px-6 py-4 text-sm font-semibold tabular-nums text-ink-0">{formatCurrency(p.amount)}</td>
                <td className="px-6 py-4"><div className="max-w-[180px] truncate text-sm font-medium text-ink-0">{p.cause}</div></td>
                <td className="px-6 py-4 text-xs font-semibold tabular-nums text-ink-5">{timeFormatter.format(new Date(p.dueDate))}</td>
    ...
    <div className="overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[900px] text-left">
          ...
          <tbody className="divide-y divide-[color:var(--hairline)]">
            {filtered.map((r) => (
              <tr key={r.id} className="group cursor-pointer bg-[var(--surface-0)] transition-colors hover:bg-[var(--surface-1)]/60 focus-within:bg-[var(--surface-1)]/80" onClick={() => openDrawer({ kind: "refund", data: r })} tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openDrawer({ kind: "refund", data: r }); } }}>
                <td className="px-6 py-4 text-sm font-semibold text-ink-0">{r.reference}</td>
                <td className="px-6 py-4 text-sm font-medium text-ink-0">{r.customer}</td>
                <td className="px-6 py-4 text-xs font-semibold tabular-nums text-ink-5">{timeFormatter.format(new Date(r.date))}</td>
                <td className="px-6 py-4 text-sm font-semibold tabular-nums text-[color:var(--signal-danger)]">{formatCurrency(r.amount)}</td>
                <td className="px-6 py-4"><div className="max-w-[160px] truncate text-sm font-medium text-ink-0">{r.reason}</div></td>
                <td className="px-6 py-4 text-sm font-medium text-ink-5">{r.method}</td>
      ...
      <div className="overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[700px] text-left">
            ...
            <tbody className="divide-y divide-[color:var(--hairline)]">
              {filtered.map((c) => (
                <tr key={c.id} className="group cursor-pointer bg-[var(--surface-0)] transition-colors hover:bg-[var(--surface-1)]/60 focus-within:bg-[var(--surface-1)]/80" onClick={() => openDrawer({ kind: "commission", data: c })} tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openDrawer({ kind: "commission", data: c }); } }}>
                  <td className="px-6 py-4 text-sm font-semibold text-ink-0">{c.source}</td>
                  <td className="px-6 py-4"><ExportTypeBadge type={c.type} /></td>
                  <td className="px-6 py-4 text-sm font-semibold tabular-nums text-ink-0">{formatCurrency(c.amount)}</td>
                  <td className="px-6 py-4 text-sm font-medium tabular-nums text-ink-5">{c.percentage}%</td>
                  <td className="px-6 py-4 text-sm font-semibold tabular-nums text-ink-0">{c.transactions}</td>
                  <td className="px-6 py-4 text-xs font-medium text-ink-5">{c.period}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-left">
            ...
            <tbody className="divide-y divide-[color:var(--hairline)]">
              {filtered.map((m) => (
                <tr key={m.id} className="group cursor-pointer bg-[var(--surface-0)] transition-colors hover:bg-[var(--surface-1)]/60 focus-within:bg-[var(--surface-1)]/80" onClick={() => openDrawer({ kind: "margin", data: m })} tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openDrawer({ kind: "margin", data: m }); } }}>
                  <td className="px-6 py-4"><div className="max-w-[200px] truncate text-sm font-semibold text-ink-0">{m.name}</div></td>
                  <td className="px-6 py-4 text-sm font-medium text-ink-5">{m.category}</td>
                  <td className="px-6 py-4 text-sm font-semibold tabular-nums text-ink-0">{formatCurrency(m.revenue)}</td>
                  <td className="px-6 py-4 text-sm font-medium tabular-nums text-ink-5">{formatCurrency(m.cost)}</td>
                  <td className="px-6 py-4 text-sm font-semibold tabular-nums text-[color:var(--signal-success)]">{formatCurrency(m.margin)}</td>
                  <td className="px-6 py-4 text-sm font-semibold tabular-nums text-ink-0">{m.marginPercent}%</td>
...
function SummaryCard({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className={cn("rounded-[var(--r-md)] border border-[color:var(--hairline)] p-5 ", accent ? "bg-ink-0" : "bg-[var(--surface-0)]")}>
      <p className={cn("text-[11px] font-semibold uppercase tracking-[0.18em]", accent ? "text-ink-6" : "text-ink-5")}>{label}</p>
      <p className={cn("mt-2 text-2xl font-semibold tracking-tight", accent ? "text-white" : "text-ink-0")}>{value}</p>
    </div>
  );
}
...
function NavCard({ icon, title, description, onClick }: { icon: React.ReactNode; title: string; description: string; onClick: () => void }) {
  return (
    <button className="group flex items-start gap-4 rounded-[var(--r-md)] border border-[color:var(--hairline)] bg-[var(--surface-0)] p-5 text-left  transition-all focus-visible:outline-none focus-visible:shadow-[var(--shadow-focus)]" onClick={onClick} type="button">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[var(--r-sm)] bg-[var(--surface-1)] transition-colors group-hover:bg-[var(--surface-2)]">{icon}</div>
      <div className="min-w-0">
        <p className="text-sm font-semibold text-ink-0">{title}</p>
        <p className="mt-1 text-xs font-medium text-ink-5">{description}</p>
      </div>
...
function ToolbarSelect({ icon, label, onChange, options, value }: { icon: React.ReactNode; label: string; onChange: (v: string) => void; options: string[]; value: string }) {
  return (
    <label className="flex min-w-[170px] items-center gap-2 rounded-[var(--r-sm)] border border-[color:var(--hairline)] bg-[var(--surface-0)] px-3 py-2.5 text-[13px] font-semibold text-ink-4 ">
      <span className="shrink-0 text-ink-6">{icon}</span>
      <span className="text-ink-5">{label}</span>
      <select className="w-full bg-transparent text-right font-semibold text-ink-0 outline-none" onChange={(e) => onChange(e.target.value)} value={value}>
...
function TableFooter({ count, total }: { count: number; total: number }) {
  return (
    <div className="flex items-center justify-between border-t border-[color:var(--hairline)] bg-[var(--surface-1)] px-6 py-3">
      <span className="block text-[10px] font-medium uppercase tracking-[0.14em] text-ink-5">Mostrando <b className="px-1 text-ink-0 font-semibold tabular-nums">{count}</b> de <span className="tabular-nums">{total}</span></span>
    </div>
  );
}
