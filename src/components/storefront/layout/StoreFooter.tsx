      ...
      <div className="mx-auto max-w-7xl px-4 pb-8 pt-16 sm:px-6 lg:px-8 lg:pt-24">
        ...
        <div className="mt-12 flex flex-col items-center border-t border-gray-200 pt-8 sm:mt-16 bg-gray-50/50">
          <div className="flex flex-wrap justify-center gap-6 mb-8 mt-2">
            <Link href={storePath(config.slug, "legal?policy=privacy")} className="text-xs font-semibold text-gray-500 hover:text-gray-900 transition-colors uppercase tracking-wider">Politica de Privacidad</Link>
            <Link href={storePath(config.slug, "legal?policy=terms")} className="text-xs font-semibold text-gray-500 hover:text-gray-900 transition-colors uppercase tracking-wider">Terminos y Condiciones</Link>
            <Link href={storePath(config.slug, "legal?policy=refunds")} className="text-xs font-semibold text-gray-500 hover:text-gray-900 transition-colors uppercase tracking-wider">Politica de Devoluciones</Link>
          </div>

          <div className="w-full max-w-sm">
            <Link href={storePath(config.slug, "arrepentimiento")} className="group flex items-center justify-center gap-2 w-full px-4 py-3 bg-white border border-gray-300 rounded-lg shadow-sm hover:shadow-md hover:border-black transition-all">
              <span className="w-2 h-2 rounded-full bg-red-500 group-hover:animate-pulse" />
              <span className="text-sm font-bold text-gray-900 tracking-tight">Boton de Arrepentimiento</span>
            </Link>
            <p className="text-[10px] text-center text-gray-400 mt-3 font-medium">Cumplimiento Resolucion 424/2020 Secretaria de Comercio Interior</p>
          </div>
        </div>
